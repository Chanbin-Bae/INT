...
header intl4_shim_t {
    ...
    bit<4> quan_flag_q;              // Chanbin_Encoding level for current hop
    bit<4> quan_flag_hop;              // Chanbin_Encoding level for current hop
}

// const bit<16> INT_SHIM_HEADER_SIZE = 4;
const bit<16> INT_SHIM_HEADER_SIZE = 5; // Chanbin_QINT
...
header int_header_t {
    ...
    bit<4>  instruction_mask_0811; // Chanbin_Use for QINT for now 20231212
...
header tcp_length_t {//Chanbin_QINT
    bit<16> length_;
}
...
// Chanbin_Define QINT headers

header QINT_header_t {
    bit<32> QINT_ttl_header;
}
...
struct headers {
    ...
    udp_t			            udp;
    tcp_t			            tcp;
    tcp_length_t                tcp_length_; //Chanbin_QINT
    ...
    // Chanbin_QINT Metadata
    QINT_header_t QINT1;
    QINT_header_t QINT1_hop;
    ...
    int_data_t                  int_data; // previous data
...
struct QINT_meta_t{
    ...
    bit<32> interval1_hop;
    bit<32> interval2_hop;
    bit<32> interval3_hop;
    bit<32> interval4_hop;
    bit<32> interval5_hop;
    bit<32> QINT_meta_hop;
