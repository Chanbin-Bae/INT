...
control process_int_source (
    ...
    action int_source(bit<5> hop_metadata_len, bit<8> remaining_hop_cnt, bit<4> ins_mask0003, bit<4> ins_mask0407, bit<4> ins_mask0811) {
        ...
        hdr.intl4_shim.quan_flag_q = 0;                           // Chanbin_QINT
        hdr.intl4_shim.quan_flag_hop = 0;                           // Chanbin_QINT
        ...
        hdr.int_header.instruction_mask_0811 = ins_mask0811; // not supported Chanbin_Used for QINT
        ...
        if(hdr.tcp.isValid()){
            hdr.tcp_length_.length_ = hdr.tcp_length_.length_+ INT_TOTAL_HEADER_SIZE;
        }
