...
parser MyParser(packet_in packet,
    ...
    state parse_tcp {
        ...
        transition select(hdr.ipv4.dscp) {
            DSCP_INT &&& DSCP_MASK: parse_intl4_shim; //Chanbin_QINT
            default: accept;
        }
    }
    ...
    state parse_intl4_shim {
        ...
        // ToDo : QINT parser with quan_flag
        // local_metadata.int_meta.quan_lvl = hdr.intl4_shim.quan_flag;
        transition parse_int_header;
    }

    state parse_int_header {
        packet.extract(hdr.int_header);
        transition select(hdr.int_header.instruction_mask_0811){
            3: parse_QINT_hdr;
    ...
    state parse_QINT_hdr {
        packet.extract(hdr.QINT1);
        packet.extract(hdr.QINT1_hop);
        transition parse_int_data;
    }

    // ToDo: parser state for QINT 
...
control MyDeparser(packet_out packet, in headers hdr) {
    apply {           
        ...
        packet.emit(hdr.udp);
        packet.emit(hdr.tcp);
        packet.emit(hdr.tcp_length_); //Chanbin_QINT

        // int header
        packet.emit(hdr.intl4_shim);
        packet.emit(hdr.int_header);
        // hop metadata
        packet.emit(hdr.QINT1);
        packet.emit(hdr.QINT1_hop);
